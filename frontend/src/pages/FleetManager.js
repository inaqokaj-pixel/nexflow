...
const VEHICLE_TYPES = ['truck', 'van', 'semi_truck', 'container', 'cargo_plane', 'motorcycle', 'ship', 'train'];
const VEHICLE_EMOJI = { truck: '🚛', van: '🚐', semi_truck: '🚚', container: '📦', cargo_plane: '✈️', motorcycle: '🏍️', ship: '🚢', train: '🚂' };
const WEIGHT_PRESETS = [500, 1000, 2000, 5000, 10000, 20000];
const VOLUME_PRESETS = [5, 10, 20, 40, 80, 120];
              ...
              <select className="input" value={form.vehicle_type} onChange={e => set('vehicle_type', e.target.value)}>
                {VEHICLE_TYPES.map(t => (
                  <option key={t} value={t}>
                    {VEHICLE_EMOJI[t]} {t.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}
                  </option>
                ))}
              </select>
            </div>
          ...
          {vehicles.map(v => (
            <div key={v._id} className="card">
              <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
                <div style={{ fontSize: 36 }}>{VEHICLE_EMOJI[v.vehicle_type] || '🚛'}</div>
                <div>
                  <div style={{ fontWeight: 600, textTransform: 'capitalize', fontSize: 15 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {[
                  ...
                  ['Available from', v.availability?.available_from
                    ? new Date(v.availability.available_from).toLocaleDateString()
                    : 'Now'],
                ].map(([k, val]) => (
