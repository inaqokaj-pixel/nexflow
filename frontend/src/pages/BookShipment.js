...
export default function BookShipment() {
  ...
  useEffect(() => { api.getFleet().then(r => setVehicles(r.vehicles || [])).catch(() => {}); }, []);
  ...
  async function handleBook(e) {
    ...
    try {
      ...
      const data = await api.createBooking({
        ...
        cargo_details: { weight_kg: Number(form.weight_kg), description: form.description },
        pickup_date: form.pickup_date,
        estimated_delivery: form.estimated_delivery,
      });
      setBookingResult(data.booking); setStep(2);
  ...
  async function handlePayment() {
    ...
    try {
      const data = await api.processPayment({
        booking_id: bookingResult.id, user_id: user.id,
        amount: Number(bookingResult.total_cost),
        payment_method: paymentMethod,
       card_token: 'tok_visa',
      });
      setPaymentResult(data.payment); setStep(3);
    } catch (err) { setError(err.message); }
    finally { setLoading(false); }
  }
  ...
  return (
    <div className="page" style={{ maxWidth: 860 }}>
      <button onClick={() => navigate('/dashboard')} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', fontSize: 13, marginBottom: 20, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
        ← Back to Dashboard
      </button>
      ...
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: 32 }}>
        {steps.map((s, i) => (
          <React.Fragment key={s}>
            ...
            {i < steps.length - 1 && <div style={{ flex: 1, height: 1, background: step > i + 1 ? 'var(--success)' : 'var(--border)', margin: '0 12px', transition: 'background 0.3s' }} />}
          </React.Fragment>
        ))}
      </div>
        ...
        <form onSubmit={handleBook}>
          ...
          <div className="card" style={{ marginTop: 20 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              <div><label className="label">Weight (kg)</label><input className="input" type="number" placeholder="500" value={form.weight_kg} onChange={e => set('weight_kg', e.target.value)} required /></div>
              <div><label className="label">Description</label><input className="input" placeholder="Electronics, furniture..." value={form.description} onChange={e => set('description', e.target.value)} /></div>
            </div>
          </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 12 }}>
                {vehicles.map(v => (
                  <div key={v._id} onClick={() => set('vehicle_id', v._id)} style={{
                    ...
                    <div style={{ fontWeight: 600, fontSize: 13, textTransform: 'capitalize', marginBottom: 2 }}>{v.vehicle_type}</div>
                    <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 6 }}>{v.capacity?.weight_kg}kg · {v.current_location?.city}</div>
        ...
        <div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
            ...
            <div className="card">
              ...
              <div style={{ background: 'linear-gradient(135deg, #1a1d2e, #2d2057)', borderRadius: 12, padding: '20px 20px', marginBottom: 16, color: '#fff' }}>
      ...
      {step === 3 && paymentResult && (
        <div className="card" style={{ textAlign: 'center', padding: '60px 40px', animation: 'fadeUp 0.4s ease' }}>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
            ...
            <button className="btn btn-primary" onClick={() => { setStep(1); setBookingResult(null); setPaymentResult(null); setForm({ pickup_city:'',pickup_country:'',delivery_city:'',delivery_country:'',weight_kg:'',description:'',pickup_date:'',estimated_delivery:'',vehicle_id:'' }); }}>
              Book Another →
            </button>
          </div>
        </div>
